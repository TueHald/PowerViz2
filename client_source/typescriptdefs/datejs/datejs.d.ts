...
interface IDateJSStatic extends Date {
    /** Gets a datejs that is set to the current datejs. The time is set to the start of the day (00:00 or 12:00 AM) */
    today(): IDateJS;
    /** Compares the first datejs to the second datejs and returns an number indication of their relative values. -1 = this is lessthan datejs. 0 = values are equal. 1 = this is greaterthan datejs. */
...
interface IDateJS extends Date {
    /** Adds(or subtracts) to the value of the year, month, day, hour, minute, second, millisecond of the datejs instance using given configuration object. Positive and Negative values allowed. */
    ...
    /** Returns a new Date object that is an exact datejs and time copy of the original instance. */
    clone(): IDateJS;
    /** Determines if this instance is between a range of two dates or equal to either the start or end dates. */
    between(startDate: IDateJS, endDate: IDateJS): boolean;
    /** Compares this instance to a Date object and returns an number indication of their relative values. -1 = this is lessthan datejs. 0 = values are equal. 1 = this is greaterthan datejs. */
    ...
    /** Determines if this datejs occurs after the datejs to compare to. */
    isAfter(date: IDateJS): boolean;
    /** Determines if this datejs occurs before the datejs to compare to. */
    isBefore(date: IDateJS): boolean;
    /** Determines if the current Date instance occurs on the same Date as the supplied 'datejs'. */
    isToday(date: IDateJS): boolean;
    /** Returns the number of milliseconds between this datejs and datejs. */
    ...
    getTimezone(): string;
    /** Get the offset from UTC of the current datejs. Returns the 4-character offset string prefixed with + or - (e.g. "-0500"). */
    getUTCOffset(): string;
    /** Get the week number. Week one (1) is the week which contains the first Thursday of the year. Monday is considered the first day of the week. */
    getWeek(): number;
    /** Get the ISO 8601 week number. Week one ("01") is the week which contains the first Thursday of the year. Monday is considered the first day of the week. */
    getISOWeek(): string;
    /** Moves the datejs to Monday of the week set. Week one (1) is the week which contains the first Thursday of the year. */
    ...
    /** Moves the datejs to the first day of the month. */
    moveToFirstDayOfMonth(): IDateJS;
    /** Moves the datejs to the last day of the month. */
    ...
    /** Moves the datejs to the next nth occurrence of the dayOfWeek starting from the beginning of the month. The number (-1) is a magic number and will return the last occurrence of the dayOfWeek in the month. */
    moveToNthOccurrence(dayOfWeek: number, occurrence: number): IDateJS;
    /** Set the value of year, month, day, hour, minute, second, millisecond of datejs instance using given configuration object. */
    set(config: IDateJSLiteral): IDateJS;
    /** Set the timezone for the current datejs using a culture - specific timezone abbreviation("PST").Note that in most JavaScript implementations, this will appear to change the time since the timezone is always based on the locale. */
    setTimezone(timezoneAbbreviation: string): IDateJS;
    /** Set the timezone for the current datejs using an offset(-0700).Note that in most JavaScript implementations, this will appear to change the time since the timezone is always based on the locale. */
    setTimezoneOffset(timezoneOffset: number): IDateJS;
    /** Converts the current datejs instance into a string with an ISO 8601 format.The datejs is converted to it's UTC value. As per the ISO 8601 specification, the string will be wrapped with double quotation marks ("). */
    toISOString(): string;
